...
- **Secrets (deployed, write-only):** `GEMINI_API_KEYS` (3 keys live), `GROQ_API_KEYS` / `OPENROUTER_API_KEYS` / `NVIDIA_API_KEYS` (provider lists, **created empty on `katzu-test` 2026-09-26** so the values can be pasted in the dashboard — the pool reports each as `idle: no_keys` in `/health` and skips it until then), `ADMIN_SECRET`, `HMAC_SECRET`, `SESSION_SECRET` (read by no code path). Dodo and NOWPayments secrets are **not set**, which is why card checkout and crypto checkout answer 503. **`GOOGLE_CLIENT_ID` was listed here previously but `wrangler secret list` showed it was never actually set** (measured 2026-09-24), which is why the ID-token `aud` check had been skipped; it is now a var and enforced.
